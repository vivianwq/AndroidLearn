...
//1.使用ScrollView添加上方和下方间距
//2.在action_move的时候判断是否在滑动距离内,加入阻尼效果
//3.手势抬起或取消时判断是否要处理,回弹,使用scroller
//4.fling事件也处理
public class OverScrollView extends FrameLayout implements View.OnTouchListener {
    ...
    private void initBounce() {
        ...
        overScrollerSpringbackTask = new Runnable() {
            ...
            public void run() {
                ...
                Log.i(TAG, "获得滑动数据" + mScroller.getCurrY());
                scrollTo(0, mScroller.getCurrY());

                if (!mScroller.isFinished()) {
                    Log.i(TAG, "获得滑动数据--还没结束" + mScroller.getCurrY());
                    post(this);
                }
            }
        };
    ...
    public static final String TAG = OverScrollView.class.getSimpleName();

    public void initChildPointer() {
        child = getChildAt(0);
        Log.i(TAG, "initChildPointer" + "加入上部和下部距离空间");
    ...
    private void initScrollView() {
        ...
        post(new Runnable() {
            public void run() {
                Log.i("OverScrollView", "滑动的top距离" + child.getPaddingTop());
    ...
    public boolean isOverScrolled() {
        boolean is = (getScrollY() < child.getPaddingTop() || getScrollY() > child.getBottom() - child.getPaddingBottom() - getHeight());
        if (is) {
            Log.i(TAG, "触摸距离在滑动回弹范围内");
        } else {
            Log.i(TAG, "触摸距离不在滑动回弹范围内");
        }
        return is;
    }
    ...
    public void scrollTo(int x, int y) {
        ...
        if (getChildCount() > 0) {
            ...
            if (x != getScrollX() || y != getScrollY()) {
                Log.i(TAG, "计算长度--是否已经到达区间内true--x" + x + "---y" + y);
                super.scrollTo(x, y);
            } else {
//                Log.i(TAG, "计算长度--是否已经到达区间内false--x" + x + "---y" + y);
            }
        }
    }
    ...
    private boolean overScrollView() {
        ...
        // scrolling between the contentTop and contentBottom
            ...
            isInFlingMode = true;
            Log.i(TAG, "计算用户手势此时是否需要滑动回弹---不需要回弹");
            return false;
        }
        ...
        Log.i(TAG, "计算用户手势此时是否需要滑动回弹--" + currScrollY + "--需要回弹--" + scrollBy);
