package com.wq.andoidlearning.window.recyclerview.lib.data;
...
public class DataProvider {
    ...
    private static ArrayList<Integer> getData(){
        ...
        TypedArray bannerImage = MyApp.myApp.getResources().obtainTypedArray(R.array.image_girls);
        for (int i = 0; i < 30 ; i++) {
    ...
    private static ArrayList<Integer> getData2(){
        ArrayList<Integer> data = new ArrayList<>();
        TypedArray bannerImage = MyApp.myApp.getResources().obtainTypedArray(R.array.data_narrow_Image);
        for (int i = 0; i < 30 ; i++) {
            int image = bannerImage.getResourceId(i, R.drawable.girl2);
            data.add(image);
        }
