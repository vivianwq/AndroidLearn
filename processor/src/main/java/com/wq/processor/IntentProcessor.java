...
public class IntentProcessor extends AbstractProcessor {
    ...
    private TypeName activityClassName = ClassName.get("android.app",
            "Activity").withoutAnnotations();
    private TypeName intentClassName = ClassName.get("android.content",
            "Intent").withoutAnnotations();
    ...
    public boolean process(Set<? extends TypeElement> set, RoundEnvironment roundEnvironment) {
        ...
        //查找所有的需要注入的类描述
        List<InjectDesc> injectDescList = findInjectDesc(set, roundEnvironment);
        //log一下
        System.out.println(injectDescList);

        for (int i1 = 0; i1 < injectDescList.size(); i1++) {
            InjectDesc injectDesc = injectDescList.get(i1);

            //创建需要注解的类的Java文件,如上面所述的IntentActivity$Binder
        ...
        //创建StaticMapper类
        createJavaFile("com.alpha.staticinject", "StaticMapper"
                , method.build());
        return false;
    }
    ...
    /**
     * 生成需要注解的类,如IntentActivity 会生成 IntentActivity$Binder
     *
     * @param injectDesc 注解的类的描述信息
    ...
    private TypeName createInjectClassFile(InjectDesc injectDesc) {
        ...
        ClassName injectedClass = ClassName.get(activityName.packageName(),
                activityName.simpleName() + "$Binder");
        ...
        //生成最终的XXX$Binder文件
        createJavaFile(injectedClass.packageName(), injectedClass.simpleName(),
                method.build());
    ...
    //查找所有的需要注入的类描述
    private List<InjectDesc> findInjectDesc(Set<? extends TypeElement> set,
                                            RoundEnvironment re) {
        Map<TypeElement, List<String[]>> targetClassMap = new HashMap<>();

        //先获取所有被StaticIntentKey标示的元素
        Set<? extends Element> elements = re.getElementsAnnotatedWith(
                StaticIntentKey.class);
        for (Element element : elements) {
            ...
            //对类做缓存,避免重复
            List<String[]> strings = targetClassMap.get(classType);
            if (strings == null) {
                strings = new ArrayList<>();
                targetClassMap.put(classType, strings);
            }
            //被注解的值,如staticName
        ...
        List<InjectDesc> injectDescs = new ArrayList<>(targetClassMap.size());
        for (Map.Entry<TypeElement, List<String[]>> entry : targetClassMap.entrySet()) {
            ...
            injectDescs.add(injectDesc);
        }
        return injectDescs;
    }

    //快速获取一个类的TypeName结构
    //className--类完整名,如java.lang.String,也可能是int,boolean
    ...
    private ClassName className(String className) {
        ...
        //手动解析 java.lang.String,分成java.lang的包名 和String的类名
    ...
    public static class InjectDesc {
        ...
        public String toString() {
            return "InjectDesc{" +
                    ...
                    ", fieldTypeNames=" + Arrays.toString(fieldTypeNames) +
